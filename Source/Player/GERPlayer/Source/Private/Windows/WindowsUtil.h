...
namespace WindowsUtil
{
	...
	float GetX2AxisInput()
	{
		bool isLeft = GetAsyncKeyState(0x41);		// A
		bool isRight = GetAsyncKeyState(0x44);		// D
		if (isLeft ^ isRight)
		{
			return isLeft ? -1.f : 1.f;
		}
	...
	float GetY2AxisInput()
	{
		bool isDown = GetAsyncKeyState(0x51);		// Q
		bool isUp = GetAsyncKeyState(0x45);			// E
		if (isDown ^ isUp)
		{
			return isDown ? -1.f : 1.f;
		}
	...
	float GetZ2AxisInput()
	{
		bool isDown = GetAsyncKeyState(0x57);		// W
		bool isUp = GetAsyncKeyState(0x53);			// S
		if (isDown ^ isUp)
		{
			return isDown ? -1.f : 1.f;
		}
